...
class UserController extends AbstractActionController
{
    ...
    public function addProviderAction()
    {
        ...
        if (! $adapter->isUserConnected()) {
            $this->flashMessenger()
                ->setNamespace('zfcuser-index')
                ->addMessage($this->failedAddProviderMessage);
        ...
        if ($this->getServiceLocator()
            ...
            ->getUseRedirectParameterIfPresent() && $redirect) {
            return $this->redirect()->toUrl($redirect);
        }
        
        return $this->redirect()->toRoute($this->getServiceLocator()
            ->get('zfcuser_module_options')
    ...
    public function providerLoginAction()
    {
        ...
        if ($this->getServiceLocator()
            ->get('zfcuser_module_options')
            ->getUseRedirectParameterIfPresent() && $this->getRequest()
            ->getQuery()
            ->get('redirect')) {
            $query = array(
        ...
        $redirectUrl = $this->url()->fromRoute('scn-social-auth-user/authenticate/provider', array(
            'provider' => $provider
        ), array(
            'query' => $query
        ));
    ...
    {
        $zfcUserLogin = $this->forward()->dispatch('zfcuser', array(
            'action' => 'login'
        ));
    ...
    {
        $zfcUserRegister = $this->forward()->dispatch('zfcuser', array(
            'action' => 'register'
        ));
    ...
    public function getMapper()
    {
        if (! $this->mapper instanceof UserProviderInterface) {
            $this->setMapper($this->getServiceLocator()
